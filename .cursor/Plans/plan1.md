...
**Chosen stack (Railway cost):** Express + TypeScript + [GraphQL Yoga]([https://the-guild.dev/graphql/yoga-server](https://the-guild.dev/graphql/yoga-server)) + `mysql2` + `ioredis` — low memory vs Nest/Apollo Server.
...
- `products` / `products(where: { include, status })` with `ProductCardBase`, attributes, `SimpleProductVariableProduct` prices `(format: RAW)`, variations for subscription cards
